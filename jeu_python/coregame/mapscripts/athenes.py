...
def banc(bg_surface, y):
    REPERTOIRE = f.resource_path("assets/img/decors/Athènes/banc.png")
    LARGEUR = 150
    HAUTEUR = 30
    POSITION_X = 10
    POSITION_Y = y * 30
...
def panneau(bas_surface, x):
    ...
    REPERTOIRE = f.resource_path("assets/img/decors/Athènes/bas_gradin.png")
    LARGEUR = 200
    HAUTEUR = 0
    POSITION_X = x
    POSITION_Y = 0
...
def refresh():
    ...
    for i in range(nbnouvcolonne):  # ajout de colonnes de bancs s'il y a de la place à droite (redimension de l'écran)
        colonne_banc(max_x + i*170)

    if min_x >= -20:
        colonne_banc(min_x - 150)
